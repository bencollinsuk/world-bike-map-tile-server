...
docker run --rm -p 8080:80 -e THREADS=12 -e ALLOW_CORS=1 -e "OSM2PGSQL_EXTRA_ARGS=-C 8192" -v $dir/run.sh:/run2.sh -v osm-data:/data/database/postgres --name world-bike-map-tile-server-cyclosm bencollinsuk/world-bike-map-tile-server-cyclosm run